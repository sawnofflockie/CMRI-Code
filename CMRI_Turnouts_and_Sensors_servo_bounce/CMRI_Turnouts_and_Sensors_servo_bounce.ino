...
#define POINT_START      1800
#define POINT_END        1200
...
servo initialiseServo(servo currentServo, int Throw, int Close, int servoNum) {
    ...
    if (currentServo.servoNum == POINT) {
    ...
    currentServo.bounce = (throwRange / 2) + Close; // Increase the divisor to decrease the amplitude of the bounce.
    ...
    for (int currentServo = 0; currentServo < NUMSERVOS; currentServo++) {
        ...
        if (myServos[currentServo].requiredState != myServos[currentServo].state) {
                    ...
                    switch (myServos[currentServo].state) {
                        ...
                        case CLOSING:
                            ...
                            if (myServos[currentServo].currentPosition > myServos[currentServo].Close) {
                                ...
                                // Reached the end of travel, hence change the state.
                                if (myServos[currentServo].servoNum == POINT) {
            ...
            if (myServos[currentServo].servoNum == POINT) {
                ...
                cmri.set_bit(8, bitState);  //Bit 8 = address 1009 in JMRI, Virtual Feedback Sensor 9
            }
...
void processOutputs() {
    ...
    for (int opNum = 0; opNum < NUMOUTPUTS; opNum++) {
        ...
        if (opReqState != opLastState[opNum]) {
            digitalWrite((46 + opNum), opReqState);
            opLastState[opNum] = opReqState;
        }
    }
...
void setup() {
    int servoStart[NUMSERVOS] = {POINT_START, SIG_HEAD_1_START, SIG_HEAD_2_START};
    int servoEnd[NUMSERVOS] = {POINT_END, SIG_HEAD_1_END, SIG_HEAD_2_END};
